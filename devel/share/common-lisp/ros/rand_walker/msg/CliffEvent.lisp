...
(cl:defmethod cl:initialize-instance :after ((m <CliffEvent>) cl:&rest args)
  ...
  (cl:unless (cl:typep m 'CliffEvent)
    (roslisp-msg-protocol:msg-deprecation-warning "using old message class name rand_walker-msg:<CliffEvent> is deprecated: use rand_walker-msg:CliffEvent instead.")))

(cl:ensure-generic-function 'sensor-val :lambda-list '(m))
(cl:defmethod sensor-val ((m <CliffEvent>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader rand_walker-msg:sensor-val is deprecated.  Use rand_walker-msg:sensor instead.")
  (sensor m))

(cl:ensure-generic-function 'state-val :lambda-list '(m))
(cl:defmethod state-val ((m <CliffEvent>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader rand_walker-msg:state-val is deprecated.  Use rand_walker-msg:state instead.")
  (state m))

(cl:ensure-generic-function 'bottom-val :lambda-list '(m))
(cl:defmethod bottom-val ((m <CliffEvent>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader rand_walker-msg:bottom-val is deprecated.  Use rand_walker-msg:bottom instead.")
  (bottom m))
...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<CliffEvent>)))
  ...
  (cl:format cl:nil "# cliff sensor~%uint8 LEFT   = 0~%uint8 CENTER = 1~%uint8 RIGHT  = 2~%~%# cliff sensor state~%uint8 FLOOR = 0~%uint8 CLIFF = 1~%~%uint8 sensor~%uint8 state~%~%# distance to floor when cliff was detected~%uint16 bottom~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'CliffEvent)))
  "Returns full string definition for message of type 'CliffEvent"
  (cl:format cl:nil "# cliff sensor~%uint8 LEFT   = 0~%uint8 CENTER = 1~%uint8 RIGHT  = 2~%~%# cliff sensor state~%uint8 FLOOR = 0~%uint8 CLIFF = 1~%~%uint8 sensor~%uint8 state~%~%# distance to floor when cliff was detected~%uint16 bottom~%~%~%"))
