...
(cl:defmethod cl:initialize-instance :after ((m <BumperEvent>) cl:&rest args)
  ...
  (cl:unless (cl:typep m 'BumperEvent)
    (roslisp-msg-protocol:msg-deprecation-warning "using old message class name rand_walker-msg:<BumperEvent> is deprecated: use rand_walker-msg:BumperEvent instead.")))

(cl:ensure-generic-function 'bumper-val :lambda-list '(m))
(cl:defmethod bumper-val ((m <BumperEvent>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader rand_walker-msg:bumper-val is deprecated.  Use rand_walker-msg:bumper instead.")
  (bumper m))
...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<BumperEvent>)))
  ...
  (cl:format cl:nil "# bumper~%uint8 LEFT   = 0~%uint8 CENTER = 1~%uint8 RIGHT  = 2~%~%# state~%uint8 RELEASED = 0~%uint8 PRESSED  = 1~%~%uint8 bumper~%uint8 state~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'BumperEvent)))
  "Returns full string definition for message of type 'BumperEvent"
  (cl:format cl:nil "# bumper~%uint8 LEFT   = 0~%uint8 CENTER = 1~%uint8 RIGHT  = 2~%~%# state~%uint8 RELEASED = 0~%uint8 PRESSED  = 1~%~%uint8 bumper~%uint8 state~%~%~%"))
